...
def count_voxel_for_coordinate(binary_mask, coord, counted, block_size=(3, 3, 3)):
    """
    Counts unique voxels for a single coordinate, updating the 'counted' array.
    
    :param binary_mask: The binary mask array.
    :param coord: The current coordinate as a tuple (x, y, z).
    :param counted: The array tracking which voxels have already been counted.
    :param block_size: The size of the block around each coordinate to check.
    :return: The count of unique voxels for the given coordinate.
    """
    x_start, y_start, z_start = check_coord_boundaries(coord, block_size, binary_mask.shape)
    x_end, y_end, z_end = x_start + block_size[0], y_start + block_size[1], z_start + block_size[2]
    ...
    block_mask = binary_mask[x_start:x_end, y_start:y_end, z_start:z_end]
    block_counted = counted[x_start:x_end, y_start:y_end, z_start:z_end]
    block_sum = np.sum(block_mask)

    new_voxels = (block_mask == 1) & (~block_counted)
    unique_voxels_count = np.sum(new_voxels)
    counted[x_start:x_end, y_start:y_end, z_start:z_end] |= new_voxels

    return unique_voxels_count, block_sum
...
def get_coorindates(data_directory, outfile_name , block_size = (3, 3, 3), min_distance = 3, threshold_abs = 2.0):
    ...
    with open(csv_file_name, mode='w', newline='') as file:
        ...
        if file.tell() == 0:
            csv_writer.writerow(['Patient ID', 'Study ID', 'Positive Length', 'Negative Length', 'Total Length', 'Tumor idx','Unique CC Count', 'Total CC Count', 'CC List Count', 'Total Size','Pos Pixel Count'])
        ...
        for patient_id in tqdm(patients, desc="Processing patients"):
            ...
            for study_id in subfolders:
                ...
                size = np.sum(seg_data)
                counted = np.zeros(seg_data.shape, dtype=bool)
                total_unique_voxels = 0
                pos_counter = 0
                neg_counter = 0
                for idx, coordinate in enumerate(candidate_coordinate):
                    x, y, z = coordinate
                    x_start, y_start, z_start = check_coord_boundaries(coordinate, block_size, seg_data.shape)
                    coordinate = (x_start, y_start, z_start)
                    component_label_block = connected_components[x_start:x_start + block_size[0], 
                                                           y_start:y_start+block_size[1], 
                                                           z_start:z_start+block_size[2]]
                    # if component_label > 0:
                        # cc_lst.append(component_label)
                    # Find all unique, non-zero component labels in the block
                    unique_labels = np.unique(component_label_block[component_label_block > 0])
                    # Append these labels to cc_lst
                    cc_lst.extend(unique_labels)
                    # cc_lst.append(component_label)
                    unique_voxels_count, block_sum = count_voxel_for_coordinate(seg_data, coordinate, counted, block_size = block_size)
                    total_unique_voxels += unique_voxels_count
                    if block_sum > 0:
                        pos_counter +=1
                    else:
                        neg_counter +=1
                    ...
                    #     pixel_count += count_unique_voxels(seg_data, checked_coord)
                    #     pos_counter +=1
                ...
                csv_writer.writerow([patient_id, study_id,  pos_coord_length, neg_coord_length, total_coord_length, max_cc, len(set(cc_lst)), len(cc_lst), str(cc_lst), size, total_unique_voxels])
...
def main(data_directory, outfile, block_size = (3, 3, 3), min_distance = 3, threshold_abs = 2.0):
    get_coorindates(data_directory, outfile, block_size = block_size, min_distance = min_distance, threshold_abs = threshold_abs)
if __name__ == "__main__":
    ...
    TH_2 = 2.0
    TH_1 = 1.0
    outfile_R_1_TH_2 = f'block_size_16_studies_summary_R{R_1}_TH_{TH_2}_length_cc_size_pix_count.csv'
    main(data_directory, outfile_R_1_TH_2, block_size = (16, 16, 16), min_distance = R_1, threshold_abs = TH_2)
    # outfile_R_1_TH_1 = f'studies_summary_R{R_1}_TH_{TH_1}_length_cc_size_pix_count.csv'
    # main(data_directory, outfile_R_1_TH_1, block_size = (3, 3, 3), min_distance = R_1, threshold_abs = TH_1)
    print('done!')
