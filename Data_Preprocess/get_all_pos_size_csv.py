...
def get_coorindates(data_directory, block_size = (3, 3, 3), min_distance = 3, threshold_abs = 2.0):
    ...
    # csv_file_name = f'studies_summary_R{min_distance}_TH_{threshold_abs}.csv'
    csv_file_name = f'studies_all_pos_size.csv'
    ...
    with open(csv_file_name, mode='w', newline='') as file:
        ...
        if file.tell() == 0:
            csv_writer.writerow(['Patient ID', 'Study ID', 'Total Size'])
        ...
        for patient_id in tqdm(patients, desc="Processing patients"):
            ...
            for study_id in subfolders:
                ...
                logger.info(f'Finished getting size for study {study_id} for patient {patient_id} to npy file')
                # pos_coord_length = pos_counter
                # neg_coord_length = neg_counter
                # total_coord_length = len(candidate_coordinate)
                csv_writer.writerow([patient_id, study_id, size])
